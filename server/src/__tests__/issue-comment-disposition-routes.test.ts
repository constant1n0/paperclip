...
describe.sequential("issue comment disposition routes", () => {
  ...
  it("captures the authz gate denial as a machine-readable warning without mutating the issue", async () => {
    ...
    // owner: the mutation gate denies with 409 issue_checked_out_by_another_agent
    // through the capture shim; the comment still posts and the issue stays put.
    const issue = makeIssue("in_progress");
    mockIssueService.getById.mockResolvedValue(issue);
    ...
    expect(res.body.disposition).toEqual({
      ...
      warning: expect.objectContaining({
        code: "transition_rejected",
        gateCode: "issue_checked_out_by_another_agent",
        message: "Issue is checked out by another agent",
        details: expect.objectContaining({
          issueId: ISSUE_ID,
          assigneeAgentId: ASSIGNEE_AGENT_ID,
