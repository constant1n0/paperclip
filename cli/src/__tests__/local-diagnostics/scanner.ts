...
const bindingMemberName = (node: ts.BindingElement): string | undefined => node.propertyName ? ts.isIdentifier(node.propertyName) || ts.isStringLiteral(node.propertyName) ? node.propertyName.text : ts.isComputedPropertyName(node.propertyName) && ts.isStringLiteral(node.propertyName.expression) ? node.propertyName.expression.text : undefined : ts.isIdentifier(node.name) ? node.name.text : undefined;
const assignmentMemberName = (node: ts.PropertyAssignment): string | undefined => ts.isIdentifier(node.name) || ts.isStringLiteral(node.name) ? node.name.text : ts.isComputedPropertyName(node.name) && ts.isStringLiteral(node.name.expression) ? node.name.expression.text : undefined;
const isDynamicComputedName = (name: ts.PropertyName | undefined): boolean => !!name && ts.isComputedPropertyName(name) && !ts.isStringLiteral(name.expression) && !ts.isNumericLiteral(name.expression);
const isDestructuringAssignment = (node: ts.ObjectLiteralExpression): boolean => {
  let outer: ts.Node = node;
  while (ts.isPropertyAssignment(outer.parent) || ts.isObjectLiteralExpression(outer.parent) || ts.isArrayLiteralExpression(outer.parent) || ts.isParenthesizedExpression(outer.parent) || ts.isSpreadElement(outer.parent)) outer = outer.parent;
  return ts.isBinaryExpression(outer.parent) && outer.parent.operatorToken.kind === ts.SyntaxKind.EqualsToken && outer.parent.left === outer;
};
  ...
  for (const input of sources) {
    ...
    const visit = (node: ts.Node): void => {
      ...
      if (ts.isBindingElement(node) && ts.isObjectBindingPattern(node.parent)) {
        ...
        if (member && BANNED_MEMBERS.has(member)) findings.push(finding(source, node, "B2", `banned-member:${member}`, null));
        if (isDynamicComputedName(node.propertyName)) findings.push(finding(source, node, "B2", "computed-member-access", null));
      }
      if ((ts.isPropertyAssignment(node) || ts.isShorthandPropertyAssignment(node)) && ts.isObjectLiteralExpression(node.parent) && isDestructuringAssignment(node.parent)) {
        const member = ts.isShorthandPropertyAssignment(node) ? node.name.text : assignmentMemberName(node);
        if (member && BANNED_MEMBERS.has(member)) findings.push(finding(source, node, "B2", `banned-member:${member}`, null));
        if (ts.isPropertyAssignment(node) && isDynamicComputedName(node.name)) findings.push(finding(source, node, "B2", "computed-member-access", null));
      }
